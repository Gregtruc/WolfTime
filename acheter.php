...
if (isset($_GET['cid']) and ($_SESSION['anciennete']>=3 or $_SESSION['assoc']))
{
	...
	else 
	{
		...
		$message = linkanimal($row).' a �t� vendu � <a class=atype5 href=fiche.php?uid='.$_SESSION['uid'].'>'.$_SESSION['compte'].'</a> au prix de '.$prix.'<img src=monnaie.png>';
		if ($remise)
		...
		{
			$message=$_SESSION['fichemoi'].' a achet� '.linkanimal($row).' � <a class=atype5 href=fiche.php?uid='.$row[1].'>'.$row[4].'</a> pour '.$row[0].'<img src=monnaie.png>.';
...
L'hotel des ventes pr�sente les ventes directes de <?=$ANIMAUX?>. 
<?php 
if ($_SESSION['anciennete']>=3 or $_SESSION['assoc'])
	echo '<a class=atype5 href = "listevente2.php">Acc�der � l\'hotel des ventes </a>';
else 
	echo '<br><br><span class=important>Disponible � partir de 3 jours d\'anciennet�.</span>';

echo finFenetre();
echo debutFenetre('La salle des ench�res');?>
La salle des ench�res regroupe les ench�res de <?=$ANIMAUX?>.
<!-- definit si l'utilisateur a acces a la salle des encheres --> 
<?php 
if ($_SESSION['anciennete']>=3 or $_SESSION['assoc'])
	echo '<a class=atype5 href = "encheres.php">Acc�der � la salle des ench�res </a>';
else 
	echo '<br><br><span class=important>Disponible � partir de 3 jours d\'anciennet�.</span>';
echo finFenetre();
...
echo 'Cette partie '.mot('venteanimaux','ofthe').' permet '.mot('association','to',0,1).' de racheter '.mot('animal','a',0,1).' c�d�s par des joueurs. <a class=atype5 href=secondevie.php>Acc�der � la filiale seconde vie '.mot('venteanimaux','ofthe').'</a>';
echo finFenetre();
}
endif;
...
if (isset($_POST['sexe']) and isset($_POST['race']) and $_POST['race']!='' and $_POST['sexe']!='') 
{
	...
	echo '<a class="atype2" href="acheter.php">R�initialiser le choix</a>';
	echo '<br>';
	...
	if ($SITE_ID==2 and !$inforace['envente'])
		...
		echo 'Sexe : '.(($_POST['sexe']==1) ? 'M�le' : 'Femelle').'<br>';
...
else if (isset($_GET['race']))
{
	...
	echo '<form method="POST" name="choix">
		...
		echo '<td width = 100px><input type="radio" name="sexe" value="1"><a class="atypespe" href="#"  onclick="document.choix.sexe[0].checked=\'checked\'; return false;">M�le</a></td> 
...
{
	echo '<span class=important>Les robes pr�sent�es sur cette page sont des exemples de robe disponible pour chaque race, d\'autres robes sont souvent disponibles.</span><br><br>Choisissez votre race :';
    ...
    {
    	echo '<br><span class=combleu>Races Sp�ciales</span>';
...
{
	if ($_GET['erreur']==0)
		echo '<script language="javascript">alert("Vous avez bien achet� ce '.mot('animal').'.")</script>';
	else if ($_GET['erreur']==1)
		echo '<script language="javascript">alert("Ce '.mot('animal').' n\'est pas � vendre.")</script>';
	else if ($_GET['erreur']==2)
		echo '<script language="javascript">alert("Vous �tes d�j� le propri�taire de ce '.mot('animal').'.")</script>';
	else if ($_GET['erreur']==3)
		echo '<script language="javascript">alert("Vous n\'avez pas assez d\'argent.")</script>';
	else if ($_GET['erreur']==4)
		echo '<script language="javascript">alert("Cette vente ne vous est pas r�serv�e.")</script>';
	else if ($_GET['erreur']==5)
		echo '<script language="javascript">alert("'.$LASSOCIATION.' ne g�re pas ces '.$ANIMAUX.'.")</script>';
	// #################### DEBUT MODIF 2 ################################################################################
